    ...
    def forward(self, x, mask = None):
        if self.mode == 'ViT':
            for attn, ff in self.layers:
                x = attn(x, mask = mask)
                x = ff(x)
        elif self.mode == 'CAF': # Cross-layer Adaptive Fusion
            last_output = []
            nl = 0
            for attn, ff in self.layers:           
...
class SpectralFormerHashNet(nn.Module):
    def __init__(self, in_channels, hash_bit_length, patch_size=11, dim=64, depth=5, heads=4, mlp_dim=8, pool='cls', dim_head=16, dropout=0.1, emb_dropout=0.1, mode='CAF'):
        super().__init__()

        # For SpectralFormer, patches correspond to spectral bands
